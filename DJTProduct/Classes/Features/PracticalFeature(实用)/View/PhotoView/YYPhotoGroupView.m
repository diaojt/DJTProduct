...
- (void)presentFromImageView:(UIView *)fromView
    ...
    _snapshotImage = [_toContainerView.viewController.tabBarController.view snapshotImageAfterScreenUpdates:YES];
    BOOL fromViewHidden = fromView.hidden;
    fromView.hidden = YES;
    _snapshorImageHideFromView = [_toContainerView.viewController.tabBarController.view snapshotImage];
    ...
    toContainer.viewController.hideNum = [NSNumber numberWithInteger:1];
    [toContainer.viewController prefersStatusBarHidden];
    [toContainer.viewController setNeedsStatusBarAppearanceUpdate];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    // 控制导航栏
    _toContainerView.viewController.hideNum = [NSNumber numberWithBool:_fromNavigationBarHidden];
    [_toContainerView.viewController prefersStatusBarHidden];
    [_toContainerView.viewController setNeedsStatusBarAppearanceUpdate];
    ...
    [UIView animateWithDuration:animated ? 0.2 : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (isFromImageClipped) {
            ...
            cell.imageView.frame = fromFrame;
            cell.imageView.cst_y += kStatusBarHeight;
        }
    }completion:^(BOOL finished) {
...
- (void)pan:(UIPanGestureRecognizer *)g {
    ...
    switch (g.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            _scrollView.left = deltaX;
            
            CGFloat alphaDelta = 160;
            CGFloat delta = MAX(fabs(deltaY), fabs(deltaX));
            //CGFloat alpha = (alphaDelta - fabs(deltaY) + 50) / alphaDelta;
            CGFloat alpha = (alphaDelta - delta + 50) / alphaDelta;
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            CGFloat delta1 = MAX(fabs(deltaY), fabs(deltaX));
            CGFloat delta2 = MAX(fabs(v.y), fabs(v.x));
            
            if (delta2 > 1000 || delta1 > 120) {
                [self cancelAllImageLoad];
                weakSelf.isPresented = NO;
                
                BOOL moveToTop = (v.y < - 50 || (v.y < 50 && deltaY < 0));
                BOOL moveToLeft = (v.x < - 50 || (v.x < 50 && deltaX <0));
                ...
                CGFloat duration2 = (moveToLeft ? _scrollView.right : self.width - _scrollView.left) / vx;
                
                duration *= 0.8;
                duration2 *= 0.8;
                
                duration = YY_CLAMP(MAX(duration, duration2), 0.05, 0.3);
                
                [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionBeginFromCurrentState animations:^{
                    weakSelf.blurBackground.alpha = 0;
                    weakSelf.pager.alpha = 0;
                    if (moveToTop) {
                        weakSelf.scrollView.bottom = 0;
                    } else {
                        weakSelf.scrollView.top = self.height;
                    }
                    if (moveToLeft) {
                        weakSelf.scrollView.right = 0;
                    }else{
                        weakSelf.scrollView.left = self.width;
                    }
                    
                } completion:^(BOOL finished) {
                    [self removeFromSuperview];
                    
                    weakSelf.toContainerView.viewController.hideNum = [NSNumber numberWithBool:weakSelf.fromNavigationBarHidden];
                    [weakSelf.toContainerView.viewController prefersStatusBarHidden];
                    [weakSelf.toContainerView.viewController setNeedsStatusBarAppearanceUpdate];
                }];
                ...
                [UIView animateWithDuration:0.4 delay:0 usingSpringWithDamping:0.9 initialSpringVelocity:v.y / 1000 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
                    weakSelf.scrollView.top = 0;
                    weakSelf.scrollView.left = 0;
